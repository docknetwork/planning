...
## Motivation
Schemas help the holders and verifiers set the expectations for the data contained in a credential. 
...
To write a schema on chain, send the following JSON document on chain with unique schema id and the DID's signature. The JSON document is serialized using JCS serialization.
```json
{
...
Any credential specifying the above as schema will have it `credentialSubject` be compliant with the schema.   
